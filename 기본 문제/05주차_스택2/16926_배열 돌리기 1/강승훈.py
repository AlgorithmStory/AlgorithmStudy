# 기본적으로 테두리를 1차원 배열로 다 가져온다음, r 시키고, 다시 집어넣는 순이라고 생각하시면 됩니다.
...
def arr_get(n,m,bias): # 처음 매개변수로 n과 m을 넣어주지만, 테두리를 안으로 들어가면서 여러번 가져와야 하기 때문에 함수 실행 구문 보면, b라는 녀석을 추가해줬음.
    ...
    index_2 = 0
    for i in range((n * m) - ((n - 2) * (m - 2))): # 포문 조건식은 가장 바깥쪽의 테두리의 길이를 계산 한거임. (즉, 백준 예제 1번에서 이 함수가 처음 실행될땐, 12가 되겠죠.)
    ...
    table_arr.append(table_arr_sub) # 포문이 끝났으면 임시 저장된 배열을, 최종 배열에 담음.
